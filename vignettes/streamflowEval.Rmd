...
vignette: >
  %\VignetteIndexEntry{Evaluate streamflow simulation with rwrfhydro}
...
## get the lon, lat of the frxst_pts_out. The plyr approach works if there is more than one frxst point.
library(plyr)
fcLocation <- ddply(modStrh.chrt.fc, .(st_id), function(df) df[1,c('st_lon','st_lat')])
near <- FindUsgsStns(stnLon=c(fcLocation$st_lon), stnLat=c(fcLocation$st_lat), within=.01)

dbPath <- '~/wrfHydroTestCases/usgsDb/'
QuerySiteInfo(c('station_nm','site_no','dec_lat_va','dec_long_va'), path=dbPath)
## show how to match lat,lon of frxst pt instead of just knowing the name/site_no

obsStr5min.fc <- PrettySiteData(QuerySiteData(QuerySiteName("FOURMILE CREEK AT ORODELL, CO", path=dbPath), 
