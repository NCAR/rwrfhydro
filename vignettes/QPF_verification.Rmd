...
MRMS<-list.files(path=pathMRMS,full.names = TRUE, pattern = "PRECIP_FORCING")
```
...
sg$rainGgaugeInds <- GetGeogridIndex(data.frame(lon=sg$longitude, lat=sg$latitude),
                                    geoGridAdd)
```
...
```{r}
    ...
    #just keep teh value and time to make the size small
...
Take the time stamp from the file name:
