...
```{r}
modLdasoutWb1h.allrt.fc <- ReadLdasoutWb(paste0(dataPath, '/RUN.RTTESTS/OUTPUT_ALLRT'), 
...
```{r}
modRtout1h.allrt.fc <- ReadRtout(paste0(dataPath, '/RUN.RTTESTS/OUTPUT_ALLRT'), 
                                 paste0(dataPath, '/DOMAIN/hydro_OrodellBasin_100m.nc'), 
...
```{r}
modGwout.allrt.fc <- ReadGwOut(paste0(dataPath, '/RUN.RTTESTS/OUTPUT_ALLRT/GW_outflow.txt'))
```
