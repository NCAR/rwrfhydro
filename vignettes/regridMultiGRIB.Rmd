...
Note that the user will need to provide a weight file path for each file group. In this example,
a weight file path needs to be provided for the GFS and HRRR data independtly. rwrfhydro will
check to see if the weight files exist. If they do not exist, they will be generated. So, 
provide a weight file path regardless of if it exists or not. It's recommended to name weight
files that are unique to a particular product and WRF-Hydro modeling domain.

```{r, eval=FALSE}
wghtFilesHRRR <- list(wghtFile='./rio_hrrr_wghts.Rdata')
wghtFilesGFS <- list(wghtFile='./rio_gfs_wghts.nc')
wghtList <- list(listHRRR=wghtFilesHRRR,listGFS=wghtFilesGFS)
```

# Input Files, Variables, Levels, and Level Types

This section outlines setting up lists of variables, level types, and level
...
We will setup lists for the HRRR model. If you are unsure of varialbe
...
varsHRRR <- list(var1='2t')
levelTypesHRRR <- list(levType1='heightAboveGround')
levelsHRRR <- list(level1=2)
```

Next, setup the lists for GFS.
```{r, eval=FALSE}
filesGFS <- list.files(path='./GFS',pattern='gfs.',full.names=TRUE)
varsGFS <- list(var1='2t')
levelTypesGFS <- list(levType1='heightAboveGround')
levelsGFS <- list(level1=2)
```
...
as the function will break if these rules aren't followed. 
```{r, eval=FALSE}
...
The first layer of the output list will specify the file groups (HRRR and GFS).

```{r, eval=FALSE}
names(dataOut$listHRRR)
names(dataOut$listGFS)
```
...
```{r, eval=FALSE}
print(dataOut$listHRRR$var1$name)
print(dataOut$listHRRR$var1$missing)
print(dataOut$listHRRR$var1$verfPOSIXct)
print(dataOut$listHRRR$var1$begVerifPOSIXct)
print(dataOut$listHRRR$var1$cyclePOSIXct)
print(dataOut$listHRRR$var1$longname)
print(dataOut$listHRRR$var1$units)
print(dataOut$listHRRR$var1$precision)
print(dataOut$listHRRR$var1$nativeFiles)
print(dataOut$listHRRR$var1$dimensionList)
print(dataOut$listHRRR$var1$data[200,200,1,1])
```