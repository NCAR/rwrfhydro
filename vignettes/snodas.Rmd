...
This is the path to the directory where you want your database to be built (it should exist):
```{r}
...
```{r, , fig.width = 12, fig.height = 6*1.5, out.width='700', out.height='525'}
library(ncdf4)
ncid<-nc_open(filename = snodasNcFile)
names(ncid$var)
swe <- ncvar_get(nc = ncid, varid = 'SWE')
nc_close(ncid)
