...
we are consider notifying users when if there have been updates each time the library is loaded).
``` R
...
Note that devtools installs rwrfhydro into you default library path (`.libPaths()[1]`) and that the sources code is not 
included. To edit the source, code you can download from github or you can actually fork and clone the repository to your 
...
allows you to load the development package similarly to as if it were a package in your library path. The basic use of devtools
is outlined below. It greatly stream lines all aspects of developing R packages and is highly recommended. 
...
* Fork project
* Development cycles:
  * Make a topic branch in git (e.g. `git checkout newBranch`)
  ...
  * Merge your master with the central repository (minimize merge pains).
...
* [My introduction to R: multipler resources but sorry the video link is broken.] (https://nex.nasa.gov/nex/resources/118/) 
