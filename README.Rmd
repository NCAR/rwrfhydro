...
so it is installed in this process. The following is done for the initial install or to update (if there are changes to 
rwrfhydro - we may consider notifying users when the library is loaded if there have been updtes).
``` R
...
# Using
After the one-time install, simply load the rwrfhydro library in your R session:
```
library(rwrfhydro)
```
