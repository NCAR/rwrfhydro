...
GetCRN <- function(gageNames = NULL, siteIds= NULL, elements = NULL, timestep = "hourly", domain = NULL, 
  ...
  urlSubHourly = "ftp://ftp.ncdc.noaa.gov/pub/data/uscrn/products/subhourly01/"
  urlHourly = "ftp://ftp.ncdc.noaa.gov/pub/data/uscrn/products/hourly02/"
  urlDaily = "ftp://ftp.ncdc.noaa.gov/pub/data/uscrn/products/daily01/"
  urlMonthly = "ftp://ftp.ncdc.noaa.gov/pub/data/uscrn/products/monthly01/"
  ...
  if (timestep == "subhourly"){
    ...
    headerInfo <- read.table(paste0(urlSubHourly,"/HEADERS.txt"), stringsAsFactors = FALSE) 
    
    urlPaths <- paste0(urlSubHourly,years,"/")
    ...
    GetsubHourlyFile <- function(file){
      dat <- tryCatch(readr::read_table(file, col_names = FALSE, col_types = paste(c(rep('c', 6), rep('d', 17)), collapse = '')),
    ...
    # reading the hear information which is distributed in a separate file
    headerInfo <- read.table(paste0(urlHourly,"/HEADERS.txt"), stringsAsFactors = FALSE) 
    
    urlPaths <- paste0(urlHourly,years,"/")
    ...
    GetHourlyFile <- function(file){
      
      dat <- tryCatch(readr::read_table(file, col_names = FALSE, col_types = paste(c(rep('c', 6), rep('d', 32)), collapse = '')),
                      error=function(cond) {message(cond); return(NA)})
      names(dat) <- headerInfo[2,]
    ...
    # reading the hear information which is distributed in a separate file
    headerInfo <- read.table(paste0(urlDaily,"/HEADERS.txt"), stringsAsFactors = FALSE) 
    
    urlPaths <- paste0(urlDaily,years,"/")
    ...
    GetDailyFile <- function(file){
      
      dat <- tryCatch(readr::read_table(file, col_names = FALSE, col_types = paste(c(rep('c', 3), rep('d', 25)), collapse = '')),
                      error=function(cond) {message(cond); return(NA)})
      names(dat) <- headerInfo[2,]
    ...
    # reading the hear information which is distributed in a separate file
    headerInfo <- read.table(paste0(urlMonthly,"/HEADERS.txt"), stringsAsFactors = FALSE) 
    
    urlPaths <- paste0(urlMonthly)
    ...
    # the monthly files have some other file rather than the CRN files, so remove those files
    filenames <- unique(grep(paste("CRN",collapse="|"), filenames, value=TRUE))
    
    # if part of the file names has been given, find only those files and open those
    if (!is.null(gageNames)) filenames <- unique(grep(paste(gageNames,collapse="|"), filenames, value=TRUE))
    
    GetMonthlyFile <- function(file){
      
      dat <- tryCatch(readr::read_table(file, col_names = FALSE, col_types = paste(c(rep('c', 3), rep('d', 12)), collapse = '')),
                      error=function(cond) {message(cond); return(NA)})
