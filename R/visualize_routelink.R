...
#' @export
VisualizeRouteLink <- function(file='Route_Link.nc', downstreamReExp='', parallel=FALSE) {
  ...
  if(downstreamReExp=='') {
    if(nrow(rl) > 15000)
      cat("****","Generating the connectivity can be very slow, you can drasctically",
          "increase speed by suppling the second argument, e.g.:",
          "downstreamReExp='Route_Link.reExpTo.Rdb',",
          "corresponding to the first argument, typically generated by",
          "ReExpressRouteLink().","****",sep='\n')
    ...
    load(downstreamReExp)
    toInds <- rl$lat * NA
    toInds[which(to$start!=0)] <- to$to[to$start[which(to$start!=0)]]
    rl$to_lat <- rl$lat[toInds]
    rl$to_lon <- rl$lon[toInds]
    rl$to_lat[which(is.na(rl$to_lat))] <- rl$lat[which(is.na(rl$to_lat))]
    rl$to_lon[which(is.na(rl$to_lon))] <- rl$lon[which(is.na(rl$to_lon))]
  }
